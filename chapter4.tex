...
Figure \ref{fig:static2_emit_comp} shows the calculation of the 95\% normalized horizontal emittance from the IPM data presented Fig. \ref{fig:static2_ipm_comp}. In this case, each point from each scatter plot is indexed by their decimated turn number. The decimated turn number is the position of the data point in the IPM return array. Even though the IPM can analyze up to 64000 turns, it can only output an array of 1024 decimated turns. The data analyzed in these plots was for 64000 turns decimated into 1024 data points. As mentioned before, this data can not be represented with an error bar plot because it does not necessarily follow a normal distribution. This can be seen on the evolution against the turn number illustrated in Fig. \ref{fig:static2_emit_comp}.  
