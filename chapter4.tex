...
\begin{equation}
    ...
    \end{pmatrix}. 
\end{equation}
Again, in order to use least-squares approximation, this calculation should take the 10 BPMs upstream and the 10 BPMs downstream of the BPM(i), whose phase space coordinates are being calculated. 
...
\end{equation}
Once $\hat{X}_0$ is calculated from the data of 10 BPMs upstream and downstream of BPM(i). The phase space coordinates $X_{BPM(i)}$ at BPM(i) can be calculated from:
\begin{equation}
    \label{eq:xbpmi}
    X_{BPM(i)} = \begin{pmatrix}
        \vec{x} \\
        \vec{x}' \\
        \vec{\delta}
    \end{pmatrix}_{BPM(i)}=
